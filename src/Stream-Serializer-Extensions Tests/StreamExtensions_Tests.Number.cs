    ...
    public partial class StreamExtensions_Tests
    {
        ...
        public void Number_Tests()
        {
            ...
            foreach (var info in data)
            {
                ...
                ms.WriteNumber(info.Number);
                Assert.AreEqual(info.Length, ms.Length, info.Number.ToString());
                ms.Position = 0;
                Assert.AreEqual(Convert.ChangeType(info.Number, info.Type), ms.ReadNumber(info.Type), $"{info.Number.GetType()} {info.Number}");
                ms.SetLength(0);
                ms.Position = 0;
            }
            ms.WriteNumberNullable(123);
            Assert.AreEqual(2L, ms.Length);
            ms.Position = 0;
            Assert.AreEqual(123, ms.ReadNumberNullable<int>());
            ms.SetLength(0);
            ms.Position = 0;
            ms.WriteNumberNullable((int?)null);
            Assert.AreEqual(1L, ms.Length);
            ms.Position = 0;
            Assert.IsNull(ms.ReadNumberNullable<int>());
        }
        ...
        public async Task NumberAsync_Tests()
        {
            ...
            {
                Assert.AreEqual(info.NumberType, SerializerHelper.GetNumberAndType(info.Number).Type, info.Number.ToString());
                await ms.WriteNumberAsync(info.Number);
                Assert.AreEqual(info.Length, ms.Length, info.Number.ToString());
                ms.Position = 0;
                Assert.AreEqual(Convert.ChangeType(info.Number, info.Type), await ms.ReadNumberAsync(info.Type), $"{info.Number.GetType()} {info.Number}");
                ms.SetLength(0);
                ms.Position = 0;
            }
            await ms.WriteNumberNullableAsync(123);
            Assert.AreEqual(2L, ms.Length);
            ms.Position = 0;
            Assert.AreEqual(123, await ms.ReadNumberNullableAsync<int>());
            ms.SetLength(0);
            ms.Position = 0;
            await ms.WriteNumberNullableAsync((int?)null);
            Assert.AreEqual(1L, ms.Length);
            ms.Position = 0;
            Assert.IsNull(await ms.ReadNumberNullableAsync<int>());
        }
    }
}
